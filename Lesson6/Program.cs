/*Знакомство с языками программирования (семинары)Урок 6. Одномерные массивы. Продолжение*/
/*
...
int[] tmparray = array; // строка 192 созданние массива типа int и запись в него ссылки на исходный массив
...
Console.WriteLine(array == copyarray); // строка 197 вывод в консоль результата сравнения ссылок на разные массивы (копии и исходного) будет false так как под них выделены отдельные участки памяти

Console.WriteLine(array == tmparray); // строка 199 вывод в консоль результата сравнения ссылок массивов которые лежат по одной ссылке. будет true так как это ссылки на один и тот же участок памяти
...
{
    int[] copyarray = new int[array.Length]; // строка 219 создаем новый массив типа int с размером равным длине исходного (переданного массива)
